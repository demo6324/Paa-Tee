...
class MaxProbExtractor(nn.Module):
    ...
    def forward(self, YOLOoutput):
        ...
        output = YOLOoutput.view(batch, 5, 5 + self.num_cls , h * w)  # [batch, 5, 85, 196]
        output = output.transpose(1, 2).contiguous()  # [batch, 85, 5, 196]
        output = output.view(batch, 5 + self.num_cls , 5 * h * w)  # [batch, 85, 980]
...
class yolov3_MaxProbExtractor(nn.Module):
    pass




class yolov5_MaxProbExtractor(nn.Module):
    """MaxProbExtractor: extracts max class probability for class from YOLO output.
    ...
    def __init__(self, cls_id, num_cls, config):
        super(yolov5_MaxProbExtractor, self).__init__()
        self.cls_id = cls_id
        self.num_cls = num_cls
        self.config = config
    ...
    def forward(self, YOLOoutput):
        ...
        # import copy
        # c=copy.deepcopy(YOLOoutput)
        # output=YOLOoutput.view(batch, 5, 85, 2499)
        output = YOLOoutput.transpose(1, 2).contiguous()  # [batch, 85, 5, 361]
...
class PatchTransformer(nn.Module):
    ...
    def forward(self, adv_patch, lab_batch, img_size, do_rotate=True, rand_loc=True):
        ...
        x_pad = (img_size - adv_patch.size(-1)) / 2
        y_pad = (img_size - adv_patch.size(-2)) / 2
        ...
        cls_ids = torch.narrow(lab_batch, 2, 0, 1)#[4,14,1]
        cls_mask = cls_ids.expand(-1, -1, 3)#[4,14,3]
        cls_mask = cls_mask.unsqueeze(-1)#[4,14,3,1]
        cls_mask = cls_mask.expand(-1, -1, -1, adv_batch.size(3))#[4,14,3,300]
        cls_mask = cls_mask.unsqueeze(-1)#[4,14,3,300,1]
        cls_mask = cls_mask.expand(-1, -1, -1, -1, adv_batch.size(4))#[4,14,3,300,300]
        msk_batch = torch.cuda.FloatTensor(cls_mask.size()).fill_(1) - cls_mask #[4,14,3,300,300]

        # Pad patch and mask to image dimensions
        mypad = nn.ConstantPad2d((int(x_pad + 0.5), int(x_pad), int(y_pad + 0.5), int(y_pad)), 0)
        adv_batch = mypad(adv_batch) #[4,14,3,640,640]
        msk_batch = mypad(msk_batch) #[4,14,3,640,640]
...
class LLVIPdataset(Dataset):
    def __init__(self,vis_dir,inf_dir,lab_dir,max_lab,imgsize,shuffle=False):
        n_infimages = len(fnmatch.filter(os.listdir(inf_dir), '*.png'))+len(fnmatch.filter(os.listdir(inf_dir), '*.jpg'))
        n_visimages = len(fnmatch.filter(os.listdir(vis_dir), '*.png')) + len(fnmatch.filter(os.listdir(vis_dir), '*.jpg'))
        n_labels = len(fnmatch.filter(os.listdir(lab_dir), '*.txt'))
        #assert n_infimages==n_visimages==n_labels, "Number of LLVIP images and number of labels don't match"
        self.len = n_infimages
        self.infimg_dir = inf_dir
        self.visimg_dir = vis_dir
        self.lab_dir = lab_dir
        self.imgsize = imgsize
        self.infimg_names = fnmatch.filter(os.listdir(inf_dir), '*.png') + fnmatch.filter(os.listdir(inf_dir), '*.jpg')
        self.visimg_names = fnmatch.filter(os.listdir(vis_dir), '*.png') + fnmatch.filter(os.listdir(vis_dir), '*.jpg')
        self.shuffle = shuffle
        self.infimg_paths = []
        self.visimg_paths = []
        for img_name in self.infimg_names:
            self.infimg_paths.append(os.path.join(self.infimg_dir, img_name))
        for img_name in self.visimg_names:
            self.visimg_paths.append(os.path.join(self.visimg_dir, img_name))
        self.lab_paths = []
        for img_name in self.infimg_names:
    ...
    def __getitem__(self, idx):
        ...
        infimg_path = os.path.join(self.infimg_dir, self.infimg_names[idx])
        visimg_path = os.path.join(self.visimg_dir, self.visimg_names[idx])
        lab_path = os.path.join(self.lab_dir, self.infimg_names[idx]).replace('.jpg', '.txt').replace('.png', '.txt')

        infimage = Image.open(infimg_path).convert('RGB')
        visimage = Image.open(visimg_path).convert('RGB')
        ...
        infimage, label = self.pad_and_scale(infimage, label)
        visimage, _ = self.pad_and_scale(visimage, label)
        transform = transforms.ToTensor()
        visimage = transform(visimage)
        infimage = transform(infimage)
        label = self.pad_lab(label)
        return visimage,infimage, label
...
class InriaDataset(Dataset):
    ...
    def __init__(self, img_dir, lab_dir, max_lab, imgsize, shuffle=True):
        ...
        n_images = n_png_images + n_jpg_images + n_jpeg_images
        n_labels = len(fnmatch.filter(os.listdir(lab_dir), '*.txt'))
    ...
    def __getitem__(self, idx):
        ...
        #image, label = self.letterbox_with_labels(image, label)
        transform = transforms.ToTensor()
